...
data_path='C:\Work\PENTA\checkout\Trunk_matlab\TESTS\qhs1T';
run_ident='qhs1T';
...
%%%%%   Set up run info
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
run_surfs=[20];      %Booz_xform (VMEC) surfaces for which to run PENTA, 
                        %  note "2" is the first usable surface.
Er_min=-100;            %Minimum Er search range value (in V/cm)
Er_max=300;             %Maximum Er search range value (in V/cm)
num_Er_pts=50;          %Number of Er points to use in test range
Method='MBT';               %'SN': Use S-N method, 'MBT': Use M-B-T method, 
...
plot_flow=1;            %Plot particle flow vs Er for each surface
...
flux_cap=1;             %  Do not allow negative radial flux coefficients                        
...
%number of K values for rectangular integration. (recommend at least 10k)
num_K_vals=10000;
...
flags.flux_cap=flux_cap;
flags.Kmin=Kmin;
flags.Kmax=Kmax;
flags.num_K_vals=num_K_vals;
...
disp(['PENTA3 run finished in ' num2str(toc(Tstart)) ' seconds.'])
