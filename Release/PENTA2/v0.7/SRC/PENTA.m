function [gamma_e_ambi,q_e_ambi,gamma_i_ambi,q_i_ambi,J_bs_ambi,Er_roots,FFmat_out,X_vec_out,J_E_tote,J_E_toti_parts,J_E_cl]=...
    PENTA(pprof_info,surf_VMEC_info,Er_range,ion_params,run_ident,data_path,B_Eprl,num_ion_species,U2)
...
for ie=1:num_Er_test
    ...
    q_e(ie)=sum(FFmat(2,:).*Xvec.').*elem_charge*Te+q_PS(1)*elem_charge*Te;
    
    for ispec=1:num_ion_species
        gamma_i(ispec,ie)=sum(FFmat(3+2*(ispec-1),:).*Xvec.')+gamma_PS(ispec);
        q_i(ispec,ie)=sum(FFmat(4+2*(ispec-1),:).*Xvec.').*elem_charge*Ti(ispec)+q_PS(ispec).*elem_charge*Ti(ispec);
    end
