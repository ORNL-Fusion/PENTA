function [gamma_e_ambi,q_e_ambi,gamma_i_ambi,q_i_ambi,J_bs_ambi,Er_roots,FFmat_out,X_vec_out,J_E_tote,J_E_toti_parts,J_E_cl]=...
    PENTA(pprof_info,surf_VMEC_info,Er_range,ion_params,run_ident,data_path,B_Eprl,num_ion_species)
%This function defines the main PENTA program.  
%
% 6/2009 JL

%if set to 1, particle fluxes vs. Er are plotted for each surface
plot_flux=1;
...
for ie=1:num_Er_test
    ...
    gamma_e(ie)=sum(FFmat(1,:).*Xvec.');
    q_e(ie)=sum(FFmat(2,:).*Xvec.').*elem_charge*Te;
    
    for ispec=1:num_ion_species
        gamma_i(ispec,ie)=sum(FFmat(3+2*(ispec-1),:).*Xvec.');
        q_i(ispec,ie)=sum(FFmat(4+2*(ispec-1),:).*Xvec.').*elem_charge*Ti(ispec);
    end
...
[Er_roots]=find_Er_roots_PENTA(gamma_e,gamma_i,Er_test_vals,Z_ion,roa_surf);
...
for iroot=1:length(Er_roots)
    ...
    gamma_e_ambi(iroot)=sum(FFmat(1,:).*Xvec.');
    q_e_ambi(iroot)=sum(FFmat(2,:).*Xvec.').*elem_charge*Te;
    
    for ispec=1:num_ion_species
        gamma_i_ambi(ispec,iroot)=sum(FFmat(3+2*(ispec-1),:).*Xvec.');
        q_i_ambi(ispec,iroot)=sum(FFmat(4+2*(ispec-1),:).*Xvec.').*elem_charge*Ti(ispec);
    end
