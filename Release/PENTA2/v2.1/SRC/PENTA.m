function [gamma_e_ambi,q_e_ambi,gamma_i_ambi,q_i_ambi,J_bs_ambi,Er_roots,FFmat_out,X_vec_out,J_E_tote,J_E_toti_parts,J_E_cl,sigma_NC2,BUprls]=...
    PENTA(pprof_info,surf_VMEC_info,Er_range,ion_params,run_ident,data_path,B_Eprl,num_ion_species,U2,num_Er_pts,plot_flux,plotit_Er_find,Smax,log_interp,use_quad,use_Ji)
...
for ie=1:num_Er_test
    ...
    [Lmat,Mmat,Nmat]=define_thermal_mats(Er_test,ns,v_ths,charges,masses,loglambda,cmul_mat,efield_mat,coef2d_mat,Smax,log_interp,use_quad);
    ...
    q_e(ie)=-( sum(FFmat(2,:).*Xvec.') + qoT_PS(1) ).*elem_charge*Te;   %Gamma1 = -q/T
    
    for ispec=1:num_ion_species
        ind1=Smax+2+(ispec-1)*2+(ispec-1)*(Smax-1);
        gamma_i(ispec,ie)=sum(FFmat(ind1,:).*Xvec.')+gamma_PS(ispec+1);
        q_i(ispec,ie)=-( sum(FFmat(ind1+1,:).*Xvec.') + qoT_PS(ispec+1) ).*elem_charge*Ti(ispec);%Gamma1 = -q/T
    end
