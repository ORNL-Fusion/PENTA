...
! 
! Author(s): J. Lore 05/25/2010 - 09/29/2010
!
Contains
...
!  cmins, cmaxes: Array of max, min value of cmul for each DKES matrix
!  emins, emaxes: Ditto for efield arrays
!  xt_c_D11,Dspl_D11 etc: Spline knots and coefficients for DKES coeffs.
!  num_c_D11,num_e_D11: Number of cmul, efield values in array
...
!  Version   Date      Comment
!  -------   ----      -------
!  1.0     09/29/2010  Original Code.  JL
! 
! Author(s): J. Lore 9/29/2010 
!
! Modules used:
...
Real(rknd),    Intent(in)  :: cmins(5)
Real(rknd),    Intent(in)  :: cmaxes(5)
Real(rknd),    Intent(in)  :: emins(5)
Real(rknd),    Intent(in)  :: emaxes(5)
Real(rknd),    Intent(in)  :: xt_c_D11(num_c_D11 + kcord)
Real(rknd),    Intent(in)  :: xt_e_D11(num_e_D11 + keord)
Real(rknd),    Intent(in)  :: Dspl_D11(num_c_D11,num_e_D11)
Real(rknd),    Intent(in)  :: xt_c_D31(num_c_D31 + kcord)
Real(rknd),    Intent(in)  :: xt_e_D31(num_e_D31 + keord)
Real(rknd),    Intent(in)  :: Dspl_D31(num_c_D31,num_e_D31)
Integer(iknd), Intent(in)  :: num_c_D11
Integer(iknd), Intent(in)  :: num_e_D11
Integer(iknd), Intent(in)  :: num_c_D31
Integer(iknd), Intent(in)  :: num_e_D31
...
Real(rknd)    ::                & ! Primary species (a) paramaters 
  ma,Ta,vta,qa,na
Real(rknd)    :: L21,L12,L22,L23  ! Thermal diffusion coefficients
...
Do ispec1 = 1_iknd, num_species
  ...
  norm_factor = vta**iTHREE*ma**iTWO/(TWO*qa**iTWO)
  ...
  L12 = energy_conv(iZERO,iZERO,use_quanc8,Kmin,Kmax,numKsteps,       &
    ...
    Dspl_D11,xt_c_D11,xt_e_D11,cmins(1),cmaxes(1),emins(1),emaxes(1), &
    num_c_D11,num_e_D11,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
  ...
  If ( Avec(ind_A + 2) == ZERO ) Then 
    L23 = ZERO
  Else
    norm_factor = - vta**iTWO*ma/(TWO*B0*qa)  ! Minus sign from D31 vs D13
    K_exp  = ONE + ONE    ! Real
    nu_exp = iZERO  ! Integer
    L23 = energy_conv(iZERO,iZERO,use_quanc8,Kmin,Kmax,numKsteps,       &
      vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
      Dspl_D31,xt_c_D31,xt_e_D31,cmins(2),cmaxes(2),emins(2),emaxes(2), &
      num_c_D31,num_e_D31,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
  Endif
...
Real(rknd),    Intent(in)  :: xt_e_D31(num_e_D31 + keord)
Real(rknd),    Intent(in)  :: Dspl_D31(num_c_D31,num_e_D31)
Real(rknd),    Intent(in)  :: xt_c_D33(num_c_D33 + kcord)
Real(rknd),    Intent(in)  :: xt_e_D33(num_e_D33 + keord)
Real(rknd),    Intent(in)  :: Dspl_D33(num_c_D33,num_e_D33)
Integer(iknd), Intent(in)  :: num_c_D31
Integer(iknd), Intent(in)  :: num_e_D31
Integer(iknd), Intent(in)  :: num_c_D33
Integer(iknd), Intent(in)  :: num_e_D33
Integer(iknd), Intent(in)  :: kcord
Integer(iknd), Intent(in)  :: keord
Real(rknd),    Intent(in)  :: Avec(num_species*3)
...
Do ispec1 = 1_iknd,num_species
  ...
  Do jval = 0_iknd,Smax
    ...
    ! First RHS term
    norm_factor = vta**iTWO*ma/(TWO*qa*B0)
    K_exp = ONE     ! Real
    nu_exp = iZERO  ! Integer
    RHS_1 = energy_conv(jval,iZERO,use_quanc8,Kmin,Kmax,numKsteps,      &
      vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
      Dspl_D31,xt_c_D31,xt_e_D31,cmins(3),cmaxes(3),emins(3),emaxes(3), &
      num_c_D31,num_e_D31,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
    ...
    If ( Avec(ind_A + 2) == 0._rknd ) Then 
      RHS_3 = ZERO
    Else
      norm_factor = vta/(TWO*B0**iTWO)
      K_exp  = HALF    ! Real
      nu_exp = iZERO  ! Integer
      RHS_3 = energy_conv(jval,iZERO,use_quanc8,Kmin,Kmax,numKsteps,      &
        vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
        Dspl_D33,xt_c_D33,xt_e_D33,cmins(4),cmaxes(4),emins(4),emaxes(4), &
        num_c_D33,num_e_D33,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
    Endif
...
Function calc_QoTs_SN(num_species,Smax,abs_Er,Temps,dens,vths,charges,      &
     masses,loglambda,use_quanc8,Kmin,Kmax,numKsteps,log_interp,cmins,      &
     cmaxes,emins,emaxes,xt_c_Drat,xt_e_Drat,Dspl_Drat,xt_c_Dex,xt_e_Dex,   &
     Dspl_Dex,num_c_D31,num_e_D31,num_c_D11,num_e_D11,kcord,keord,Avec,     &
     Bsq,B0,lmat,Flows,U2)                                                  &
Result(QoTs)
!
! Description: 
...
! 
! Author(s): J. Lore 09/10/2010  - 1/6/2011
!
!
! Modules used:
Use penta_kind_mod                  ! Import rknd, iknd specifications
...
Real(rknd),    Intent(in)  :: emins(5)
Real(rknd),    Intent(in)  :: emaxes(5)
Real(rknd),    Intent(in)  :: xt_c_Drat(num_c_D31 + kcord)
Real(rknd),    Intent(in)  :: xt_e_Drat(num_e_D31 + keord)
Real(rknd),    Intent(in)  :: Dspl_Drat(num_c_D31,num_e_D31)
Real(rknd),    Intent(in)  :: xt_c_Dex(num_c_D11 + kcord)
Real(rknd),    Intent(in)  :: xt_e_Dex(num_e_D11 + keord)
Real(rknd),    Intent(in)  :: Dspl_Dex(num_c_D11,num_e_D11)
Integer(iknd), Intent(in)  :: num_c_D31
Integer(iknd), Intent(in)  :: num_e_D31
Integer(iknd), Intent(in)  :: num_c_D11 
Integer(iknd), Intent(in)  :: num_e_D11
...
Real(rknd)    :: lab21,lab22,   & ! Friction coefficient values for PS flux
  lab11,lab12      
Real(rknd)    :: QoT_PS_Sum,    & ! PS fluxes
  Gamma_PS_sum
Real(rknd)    :: Ab1,Ab2          ! Force vector values for PS flux
...
Do ispec1 = 1_iknd, num_species
  ...
  norm_factor = na * vta**iTHREE*ma**iTWO/(TWO*qa**iTWO)
  nu_exp = iZERO  ! Integer

  K_exp = THREEHALF + ONE
  L21 = energy_conv(iZERO,iZERO,use_quanc8,Kmin,Kmax,numKsteps,       &
    vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
    Dspl_Dex,xt_c_Dex,xt_e_Dex,cmins(5),cmaxes(5),emins(5),emaxes(5), &
    num_c_D11,num_e_D11,kcord,keord,K_exp,nu_exp,norm_factor,.false.)

  K_exp = THREEHALF + TWO    ! Real
  L22 = energy_conv(iZERO,iZERO,use_quanc8,Kmin,Kmax,numKsteps,       &
    vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
    Dspl_Dex,xt_c_Dex,xt_e_Dex,cmins(5),cmaxes(5),emins(5),emaxes(5), &
    num_c_D11,num_e_D11,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
  ...
  Do kval = 0_iknd,Smax
    ...
    ! Integrate QQ
    norm_factor = na * (ma*vta/qa) * (TWO*Bsq/B0/THREE)
    nu_exp = iZERO ! Integer
    K_exp = FIVEHALF  ! Real
    Na_2k(kval+1) = energy_conv(iZERO,kval,use_quanc8,Kmin,Kmax,numKsteps, &
    vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,        &
    Dspl_Drat,xt_c_Drat,xt_e_Drat,cmins(5),cmaxes(5),emins(5),emaxes(5),   &
    num_c_D31,num_e_D31,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
  EndDo

  ! Calculate PS fluxes
  QoT_PS_Sum = 0._rknd
  Gamma_PS_Sum = 0._rknd
  ...
  Do ispec2=1,num_species
    ...
    QoT_PS_Sum = QoT_PS_Sum  + U2*(  -lab21*(Ab1 + FIVE*Ab2/TWO ) &
         + lab22*Ab2  ) * Temps(ispec2)*elem_charge/(qa*charges(ispec2))

    Gamma_PS_Sum = Gamma_PS_Sum + U2*(  lab11*(Ab1 + FIVE*Ab2/TWO ) &
         - lab12*Ab2  ) * Temps(ispec2)*elem_charge/(qa*charges(ispec2))

  EndDo ! PS flux loop

  QoT_PS(ispec1)   = QoT_PS_Sum + FIVE*Gamma_PS_Sum/TWO

  ! Calculate friction flux contributions  QQ these names aren't right for SN method
...
Real(rknd)    :: lab11,lab12,   & ! Friction coefficient values for PS flux
  lab21,lab22
Real(rknd)    :: QoT_PS_Sum       ! PS flux
Real(rknd)    :: QoT_l_sum        ! Portion due to friction (sum)
Real(rknd)    :: I_1,I_2          ! Integral factors used in calculating PS flux
Real(rknd)    :: PS_coeff         ! Coefficient in front of PS term
Real(rknd)    :: term4            ! Final term in PS equation
Real(rknd)    :: X_PS2            ! Effective PS friction drive
...
Do ispec1 = 1_iknd, num_species
  ...
  Do kval = 0_iknd,Smax
    ...
    Do ispec2 = 1_iknd,num_species
      ...
      lmat_ind1 = ( ispec1 - 1 ) * ( Smax + 1 ) + 1 ! j = 0
      lmat_ind2 = ( ispec2 - 1 ) * ( Smax + 1 ) + 1 + kval

      QoT_l_sum = Sum(c_l_vals*Ja_2*lmat(lmat_ind1:lmat_ind1+Smax,lmat_ind2)/(na*ma))
  ...
  ! Calculate flux due to direct energy convolution of DKES coefficients
  ind_A = (ispec1-1)*3+1
  mono_QoT(ispec1) = -na*( L21*Avec(ind_A) + L22*Avec(ind_A+1) + L23*Avec(ind_A+2))

  ! Calculate PS flux
  PS_coeff = na*G2*ma/(qa*B0**iTWO)
  ...
  I_1 = energy_conv(iZERO,iZERO,use_quanc8,Kmin,Kmax,numKsteps, &
    vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,        &
    Dspl_D31,xt_c_D31,xt_e_D31,cmins(3),cmaxes(3),emins(3),emaxes(3),      &
    num_c_D31,num_e_D31,kcord,keord,K_exp,nu_exp,norm_factor,.true.)

  K_exp = SEVENHALF  ! Real
  I_2 = energy_conv(iZERO,iZERO,use_quanc8,Kmin,Kmax,numKsteps, &
    vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,        &
    Dspl_D31,xt_c_D31,xt_e_D31,cmins(3),cmaxes(3),emins(3),emaxes(3),      &
    num_c_D31,num_e_D31,kcord,keord,K_exp,nu_exp,norm_factor,.true.)

  term4 = (TWO/THREE)*(Ta*elem_charge/qa)*( I_1*Avec(ind_A) + I_2*Avec(ind_A+1) )
  ...
  Do ispec2=1,num_species
    ...
    ! Call the specific species beta 
    n_beta  = dens(ispec2)
    T_beta  = Temps(ispec2)
    ...
    If ( ispec1 == ispec2 ) Then
      C_PS1 = ZERO
    Else
      C_PS1 = ( (na*dTadr+Ta*dnadr)*elem_charge/(na*qa) - &
        (n_beta*dT_betadr+T_beta*dn_betadr)*elem_charge/(n_beta*q_beta) )
    EndIf 
    ...
    QoT_PS_Sum = QoT_PS_Sum + C_PS1*(FIVEHALF*lab11- lab21) + &
      C_PS2*(FIVEHALF*lab12 - lab22)

  EndDo ! PS flux loop

  X_PS2 = -QoT_PS_Sum/(na*ma) + term4
  QoT_PS(ispec1) = X_PS2*PS_coeff
...
Real(rknd)    ::  K_exp           ! Exponent on K for convolution (ignoring 
                                  !  Maxwellian contribution)
Real(rknd)    :: lab11,lab12      ! Friction coefficient values for PS flux
Real(rknd)    :: Gamma_PS_Sum     ! PS flux summation term
Real(rknd)    :: flux_l_sum       ! Friction flux summation term
Real(rknd)    :: I_0,I_1      ! Coefficients used in calculating PS flux
Real(rknd)    :: PS_coeff         ! Coefficient in front of PS terms
Real(rknd)    :: term4            ! Last term in PS equation
Real(rknd)    :: X_PS2            ! Effective friction force used in PS flux
...
Do ispec1 = 1_iknd, num_species
  ...
  Do kval = 0_iknd,Smax
    ...
    Do ispec2 = 1_iknd,num_species
      ...
      flux_l_sum = Sum(c_l_vals*Ja_1*lmat(lmat_ind1:lmat_ind1+Smax,lmat_ind2)/(na*ma))
  ...
  ! Calculate flux due to direct energy convolution of DKES coefficients
  ind_A = (ispec1-1)*3+1
  mono_flux(ispec1) = -na*( L11*Avec(ind_A) + L12*Avec(ind_A+1) + L13*Avec(ind_A+2))
  ...
  term4 = (TWO/THREE)*(Ta*elem_charge/qa)*( I_0*Avec(ind_A) + I_1*Avec(ind_A+1) )
  ...
  Do ispec2=1,num_species
    ...
    ! Calculate PS flux coefficients
    If ( ispec1 == ispec2 ) Then
      C_PS1 = ZERO
    Else
      C_PS1 = ( (na*dTadr+Ta*dnadr)*elem_charge/(na*qa) - &
    ...
    ! Calculate components of friction matrix
    lmat_ind1 = ( ispec1 - 1 ) * ( Smax + 1 ) + 1
    lmat_ind2 = ( ispec2 - 1 ) * ( Smax + 1 ) + 1
    lab11 = lmat(lmat_ind1  ,lmat_ind2)
    lab12 = lmat(lmat_ind1  ,lmat_ind2+1)
    ! Sum over species for PS flux
    Gamma_PS_Sum = Gamma_PS_Sum + C_PS1*lab11 + C_PS2*lab12

  EndDo ! PS flux loop

  ! Calculate PS flux
  X_PS2 = -Gamma_PS_Sum/(na*ma) + term4
  Gamma_PS(ispec1) = X_PS2*PS_coeff
...
!-----------------------------------------------------------------------------
Function calc_fluxes_SN(num_species,Smax,abs_Er,Temps,dens,vths,charges,  &
     masses,loglambda,use_quanc8,Kmin,Kmax,numKsteps,log_interp,          &
...
Real(rknd)    ::  K_exp           ! Exponent on K for convolution (ignoring 
                                  !  Maxwellian contribution)
Real(rknd)    :: lab11,lab12    ! Friction coefficient values for PS flux
Real(rknd)    :: Gamma_PS_Sum   ! PS fluxes
Real(rknd)    :: Ab1,Ab2          ! Force vector values for PS flux
...
Do ispec1 = 1_iknd, num_species
  ...
  Do ispec2=1,num_species
    ...
    ! Assign forces and friction coefficients to calculate PS fluxes
    spec2_Aind = (ispec2-1)*3+1
    Ab1 = Avec(spec2_Aind)
    Ab2 = Avec(spec2_Aind+1)
  ...
  Gamma_PS(ispec1) = Gamma_PS_Sum

  ! Calculate friction flux contributions 
  ind_A = (ispec1-1)*3+1
  ! Calculate flux proportional to thermodynamic forces
  mono_flux(ispec1) = - L11*Avec(ind_A) - L12*Avec(ind_A+1)
  ! Calculate flux proportional to flows
  flow_ind1 = (ispec1-1)*(Smax+1)+1
...
Real(rknd),    Intent(in)  :: xt_e_Drat(num_e_D31 + keord)
Real(rknd),    Intent(in)  :: Dspl_Drat(num_c_D31,num_e_D31)
Real(rknd),    Intent(in)  :: xt_c_DUa(num_c_D33 + kcord)
Real(rknd),    Intent(in)  :: xt_e_DUa(num_e_D33 + keord)
Real(rknd),    Intent(in)  :: Dspl_DUa(num_c_D33,num_e_D33)
Integer(iknd), Intent(in)  :: num_c_D31
Integer(iknd), Intent(in)  :: num_e_D31
Integer(iknd), Intent(in)  :: num_c_D33
Integer(iknd), Intent(in)  :: num_e_D33
...
Do ispec1 = 1_iknd,num_species
  ...
  Do jval = 0_iknd,Smax
    ...
    ind_A = (ispec1 - 1)*3 + 1
    ind_RHS = (ispec1 - 1)*(Smax+1)+jval+1
    RHS(ind_RHS) = -Ta*elem_charge* ( RHS_1*Avec(ind_A) + RHS_2*Avec(ind_A+1) &
      - idelta(jval,0_iknd)*THREEHALF*Avec(ind_A+2) )

    ! Loop over primary Sonine index (k)
    Do kval = 0_iknd, Smax

      ! Integrate the first Ua term 
      norm_factor = na * ma*B0*vta
      K_exp  = THREEHALF    ! Real
      nu_exp = iZERO    ! Integer
 
      LHS_tmp1 = energy_conv(jval,kval,use_quanc8,Kmin,Kmax,numKsteps,    &
        vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
        Dspl_DUa,xt_c_DUa,xt_e_DUa,cmins(5),cmaxes(5),emins(5),emaxes(5), &
        num_c_D33,num_e_D33,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
      ...
      Do ispec2 = 1,num_species
        ...
        LHS_mat_1(ind1_LHS2,ind2_LHS2) = LHS_mat_1(ind1_LHS2,ind2_LHS2) - &
          B0*THREEHALF*lmat(ind1_LHS2,ind2_LHS2)
...
! 
! Author(s): J. Lore 7/16/2010 - 9/10/2010
!
! Modules used:
Use penta_kind_mod                    ! Import rknd, iknd specifications
...
Else                      ! rectangular
  ...
  ! Linearly space energy evaluation points
  Kvals = rlinspace(Kmin,Kmax,numKsteps)
  dK = Kvals(2) - Kvals(1)
  ...
  Do iK = 1, numKsteps
    Coeff_tmp =  Coeff_tmp +                                                &
      ...
      kval,K_exp,nu_exp,coeffs_j,coeffs_k,Unity_coeff)
  EndDo

  Coeff_tmp = coeff_tmp * dK
...
Function intfun(Ka,vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,       &
  ...
  K_exp,nu_exp,coeffs_j,coeffs_k,Unity_coeff)                                 &
Result(integrand)
!
! Description: 
...
integrand = kfun * Dexp(-Ka) * Dstar_val * Ka**(K_exp + 0.5_rknd) &
  * nu_perp_a**nu_exp
...
nu_ab=nb*qa2*qb2*loglambda/(ma**iTWO*va3*FOUR*pi*eps0**iTWO)
! Calculate potential
...
!-----------------------------------------------------------------------------
!+ Calculates the radial fluxes using the Taguchi method (QQ)
!-----------------------------------------------------------------------------
Function calc_fluxes_T(num_species,Smax,abs_Er,Temps,dens,vths,charges,  &
     masses,loglambda,use_quanc8,Kmin,Kmax,numKsteps,log_interp,      &
...
!
! Description: 
! This function calculates the radial fluxes using the method of 
...
!  Version   Date      Comment
!  -------   ----      -------
!  1.0     08/31/2010  Original Code.  JL
!  1.1     01/06/2011  Added convective part of PS QoT. JL
! 
! Author(s): J. Lore 8/31/2010 -1/6/2011
!
!
! Modules used:
Use penta_kind_mod                  ! Import rknd, iknd specifications
...
Real(rknd)    ::                & ! Primary species (a) paramaters 
  ma, Ta, vta, qa, na     
Real(rknd)    :: L11,L12  ! Thermal diffusion coefficients
Real(rknd)    :: c_l              ! Norm. coefficients for Sonine polys
Real(rknd)    ::  norm_factor     ! Constants (wrt K) for convolution terms
Real(rknd)    ::  K_exp           ! Exponent on K for convolution (ignoring 
                                  !  Maxwellian contribution)
Real(rknd)    :: Ja_1l
...
Real(rknd)    :: qb(num_species-1),  &  ! Paremeters for species 'b' /= 'a'
  nb(num_species-1), vtb(num_species-1) 
Real(rknd)    :: Mab_1k(num_species,Smax+1)
Real(rknd)    :: Ufric_Gamma(num_species)
Real(rknd)    :: Gamma_fric(num_species)
Real(rknd)    :: Gamma_PS(num_species)
Real(rknd)    :: mono_flux(num_species)
Real(rknd)    :: flux_Ua(num_species)
...
Do ispec1 = 1_iknd, num_species
  ...
  Do kval = 0_iknd,Smax
    ...
    ! Loop over all species (including a == b) -- True?:??? QQ -- What about qb???
    Do ispec2 = 1_iknd,num_species

      Mab_1k(ispec2,kval+1) = 0._rknd

      ! Loop over secondary Sonine index (l) and sum
      Do lval = 0,Smax

        ! Calculate the c_k coefficient
        c_l = (0.75_rknd)*Dsqrt(pi)*ifactorial(lval)/gamma_aux(lval+2.5_rknd) ! Easy place for some optimization QQ
        ...
        K_exp = ONE  ! Real
        Ja_1l = energy_conv(iZERO,lval,use_quanc8,Kmin,Kmax,numKsteps, &
          vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,      &
          Dspl_D31,xt_c_D31,xt_e_D31,cmins(3),cmaxes(3),emins(3),emaxes(3),    &
          num_c_D31,num_e_D31,kcord,keord,K_exp,nu_exp,norm_factor,.false.)

        ! QQ
        lmat_ind1 = ( ispec1 - 1 ) * ( Smax + 1 ) + 1 + lval
        lmat_ind2 = ( ispec2 - 1 ) * ( Smax + 1 ) + 1 + kval

        ! QQ
        Mab_1k(ispec2,kval+1) =  Mab_1k(ispec2,kval+1) + &
          c_l*Ja_1l*lmat(lmat_ind1,lmat_ind2)/(na*ma)
  ...
  Do ispec2=1_iknd,num_species
    ...
    Ufric_Gamma(ispec2) = -Sum(Mab_1k(ispec2,:)*Flows(flow_ind2:flow_ind2+Smax))
  EndDo 
...
EndFunction calc_fluxes_T

!-----------------------------------------------------------------------------
!+ Calculates the radial energy fluxes using the Taguchi method (QQ)
!-----------------------------------------------------------------------------
Function calc_QoTs_T(num_species,Smax,abs_Er,Temps,dens,vths,charges,  &
     masses,loglambda,use_quanc8,Kmin,Kmax,numKsteps,log_interp,      &
     cmins,cmaxes,emins,emaxes,xt_c_D31,xt_e_D31,Dspl_D31,xt_c_Dex,      &
     xt_e_Dex,Dspl_Dex,num_c_D31,num_e_D31,num_c_D11,num_e_D11,kcord,    &
     keord,Avec,lmat,Flows,U2)                                       &
...
Real(rknd)    ::                & ! Primary species (a) paramaters 
  ma, Ta, vta, qa, na     
Real(rknd)    :: L12,L21,L22      ! Thermal diffusion coefficients
Real(rknd)    :: c_l              ! Norm. coefficients for Sonine polys
Real(rknd)    ::  norm_factor     ! Constants (wrt K) for convolution terms
Real(rknd)    ::  K_exp           ! Exponent on K for convolution (ignoring 
                                  !  Maxwellian contribution)
...
! Local arrays 
Integer(iknd) :: ikeep(num_species-1)   ! Used to index species 'b' in arrays
Real(rknd)    :: Na_2k(Smax+1)
Real(rknd)    :: qb(num_species-1),  &  ! Paremeters for species 'b' /= 'a'
  nb(num_species-1), vtb(num_species-1) 
Real(rknd)    :: Mab_2k(num_species,Smax+1)
Real(rknd)    :: Ufric_QoT(num_species)
Real(rknd)    :: QoT_fric(num_species)
Real(rknd)    :: QoT_PS(num_species)
Real(rknd)    :: mono_QoT(num_species)
Real(rknd)    :: QoT_Ua(num_species)
...
Do ispec1 = 1_iknd, num_species
  ...
  Do kval = 0_iknd,Smax
    ...
    Do ispec2 = 1_iknd,num_species
      ...
      Do lval = 0,Smax
        ...
        nu_exp = iZERO ! Integer        
        K_exp = TWO  ! Real
        Ja_2l = energy_conv(iZERO,lval,use_quanc8,Kmin,Kmax,numKsteps, &
          vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,      &
          Dspl_D31,xt_c_D31,xt_e_D31,cmins(3),cmaxes(3),emins(3),emaxes(3),    &
          num_c_D31,num_e_D31,kcord,keord,K_exp,nu_exp,norm_factor,.false.)
        ...
        ! QQ
        Mab_2k(ispec2,kval+1) =  Mab_2k(ispec2,kval+1) + &
          c_l*Ja_2l*lmat(lmat_ind1,lmat_ind2)/(na*ma)
  ...
  Do ispec2=1_iknd,num_species
    flow_ind2 = (ispec2 - 1)*(Smax+1)+1
    Ufric_QoT(ispec2) =   -Sum(Mab_2k(ispec2,:)*Flows(flow_ind2:flow_ind2+Smax))
  EndDo 
...
!  1.0     07/16/2010  Original Code.  JL
! 
! Author(s): J. Lore 7/16/2010 - 8/31/2010
!
!
! Modules used:
Use penta_kind_mod                  ! Import rknd, iknd specifications
...
Do ispec1 = 1_iknd,num_species
  ...
  Do jval = 0_iknd,Smax
    ...
    ! First RHS term
    norm_factor = na * vta**iTWO*ma/(TWO*qa*B0)
    K_exp = ONE     ! Real
    nu_exp = iZERO  ! Integer
    RHS_1 = energy_conv(jval,iZERO,use_quanc8,Kmin,Kmax,numKsteps,      &
      vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
    ...
    If ( Avec(ind_A + 2) == 0._rknd ) Then 
      RHS_3 = ZERO
    Else
      norm_factor = na * vta/(TWO*B0**iTWO)
      K_exp  = HALF    ! Real
      nu_exp = iZERO  ! Integer
      RHS_3 = energy_conv(jval,iZERO,use_quanc8,Kmin,Kmax,numKsteps,      &
        vta,qa,qb,ma,na,nb,loglambda,abs_Er,num_species,vtb,log_interp,   &
    ...
    Do kval = 0_iknd, Smax
      ...
      LHS_mat_1(ind1_LHS1+jval,ind1_LHS1+kval) =   &
        idelta(jval,kval)*na*Bsq/(c_k*B0) - B0*ma/(Ta*elem_charge)*LHS_tmp1
      ...
      Do ispec2 = 1,num_species
        ...
        Do mval = 0_iknd,Smax
          ...
          LHS2_sum = LHS2_sum        &
            - B0*c_m*LHS_tmp2*lmat(lmat_ind1,lmat_ind2)/(na*Ta*elem_charge)
