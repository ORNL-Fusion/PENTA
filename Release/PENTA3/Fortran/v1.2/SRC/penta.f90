...
Allocate(Dspl_Drat(num_c_D11,num_e_D11)) 
Allocate(Dspl_DUa(num_c_D11,num_e_D11))  
...
Do iroot = 1_iknd, num_roots
  ...
  SelectCase (Method)
    ...
    Case ('T')
      ...
      Gammas_ambi(:,iroot) = calc_fluxes_T(num_species,Smax,abs_Er,Temps,dens, &
        ...
        kcord,keord,Avec,lmat,Flows_ambi(:,iroot),U2)  
    ...
    Case ('SN')
      ...
      Flows_ambi(:,iroot) = calc_flows_SN(num_species,Smax,abs_Er,Temps,dens, &
         vths,charges,masses,loglambda,B0,use_quanc8,Kmin,Kmax,numKsteps,     &
         log_interp,cmins,cmaxes,emins,emaxes,xt_c_Drat,xt_e_Drat,Dspl_Drat,  &
         xt_c_DUa,xt_e_DUa,Dspl_DUa,num_c_D31,num_e_D31,num_c_D33,num_e_D33,  &
         kcord,keord,Avec,lmat)                                                
      ! Calculate array of radial particle fluxes
      Gammas_ambi(:,iroot) = calc_fluxes_SN(num_species,Smax,abs_Er,Temps,dens, &
        vths,charges,masses,loglambda,use_quanc8,Kmin,Kmax,numKsteps,           &
        log_interp,cmins,cmaxes,emins,emaxes,xt_c_Drat,xt_e_Drat,Dspl_Drat,     &
        xt_c_Dex,xt_e_Dex,Dspl_Dex,num_c_D31,num_e_D31,num_c_D11,num_e_D11,     &
        kcord,keord,Avec,Bsq,B0,lmat,Flows_ambi(:,iroot),U2)      
      ! Calculate array of radial energy fluxes
    ...
    Case ('MBT')
      ...
      Gammas_ambi(:,iroot) = calc_fluxes_MBT(num_species,Smax,abs_Er,Temps,  &
        ...
        numKsteps,log_interp,cmins,cmaxes,emins,emaxes,xt_c_D11,xt_e_D11,    &
        Dspl_D11,xt_c_D31,xt_e_D31,Dspl_D31,num_c_D11,num_e_D11,num_c_D31,   &
        num_e_D31,kcord,keord,Avec,lmat,Flows_ambi(:,iroot),G2,B0)   
      ! Calculate array of radial energy fluxes
  ...
  upol(:,iroot) = chip*(4._rknd*pi/vol_p)*(                 &
    Flows_ambi(1:(num_species-1)*(Smax+1)+1:Smax+1,iroot) - &
    bzeta*Xvec(1:(num_species-1)*2+1:2)/(charges*chip*Bsq))
  utor(:,iroot) = psip*(4._rknd*pi/vol_p)*(                 &
    Flows_ambi(1:(num_species-1)*(Smax+1)+1:Smax+1,iroot) + &
