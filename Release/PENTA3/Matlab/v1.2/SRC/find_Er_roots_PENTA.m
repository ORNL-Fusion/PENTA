function [Er_roots]=find_Er_roots_PENTA(gamma_e,gamma_i,Er_test_vals,Z_ion,roa_surf,plotit,find_gamma_e_root)
...
if find_gamma_e_root == 1      %set the ion flux to zero to find gamma_e=0
    gamma_all_i=zeros(1,num_efield);
else
...
%     I_vr=I_vr(1);
    disp('Number of Er roots not equal to 1 or 3.')
end
...
for iroot=1:length(I_vr)
    ind=I_vr(iroot);
    if ind == 1 
        error('Please increase the Er range, a root occured at the first Er point')
    end
